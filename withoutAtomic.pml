...
proctype press(int x)
{
  do
  :: (x > 0) ->
     if
     :: (press_state == load_by_arm1 && in_press)    -> atomic { press_state = pressing; x--; }
     :: (press_state == pressing && in_press)         -> atomic { press_state = unload_by_arm2; x--; }
     :: (press_state == unload_by_arm2 && !in_press)  -> progress: atomic { press_state = load_by_arm1; x--;}
     fi
  :: (x==0) -> break;
  od
}
